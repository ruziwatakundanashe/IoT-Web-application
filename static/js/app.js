...
function renderPerformancePolarChart(data) {
    ...
    const option = {
        ...
        series: [{
            ...
            radius: '60%',
            center: ['50%', '50%'],
            data: data.sensorLabels.map((label, i) => ({
                value: data.performanceData[i],
                name: label,
                itemStyle: {
                    color: i === 0 ? '#57B5E7' : i === 1 ? '#8DD3C7' : '#FFB347'
                }
            })),
            label: {
                show: true,
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    renderPerformancePolarChart(chartData);
